...
function pdf_document(string $title, string $body, bool $landscape = false): string
{
    ...
    @page{size:A4 '.$orientation.';margin:14mm}*{box-sizing:border-box}body{margin:0;color:#29273b;font:12px/1.5 Arial,sans-serif}h1{margin:0 0 6px;font-size:25px}h2{margin:22px 0 8px;font-size:17px}h3{margin:16px 0 6px;font-size:14px}h4{font-size:13px}h5{font-size:12px}h6{font-size:11px}p{margin:5px 0}.muted{color:#706e80}.header{margin-bottom:18px;padding-bottom:12px;border-bottom:2px solid #6757df}.eyebrow{color:#6757df;font-size:10px;font-weight:bold;letter-spacing:1px;text-transform:uppercase}.meta{display:flex;flex-wrap:wrap;gap:7px;margin-top:9px}.pill{padding:3px 7px;border-radius:5px;background:#ece9ff;font-size:10px}.meta-separator{color:#8b879c;font-weight:bold}.notice{margin:12px 0;padding:9px;border-left:3px solid #6757df;background:#f5f3ff}.course-step-number{width:6%;text-align:center;white-space:nowrap}.course-skill-code{display:inline-block;margin-left:7px;padding:1px 5px;border-radius:3px;background:#ece9ff;color:#5344bb;font-size:8px;font-weight:bold;white-space:nowrap}.course-reference-legend{padding:7px 9px;border-left:3px solid #6757df;background:#f5f3ff}.course-reference-legend p{margin:0}.course-objective-legend{margin-bottom:12px}.course-skill-legend{margin-top:14px}hr.markdown-separator{margin:16px 0;border:0;border-top:1px solid #dcd9e5}.markdown-page-break{height:0;margin:0;border:0;break-after:page;page-break-after:always}table{width:100%;border-collapse:collapse}th,td{padding:8px;border:1px solid #dcd9e5;text-align:left;vertical-align:top}th{background:#ece9ff;font-size:10px;text-transform:uppercase}tr:nth-child(even){background:#fafafd}.markdown-table-wrap{margin:10px 0}.markdown-table{page-break-inside:auto}.markdown-table-weighted{table-layout:fixed}.markdown-table th,.markdown-table td{overflow-wrap:anywhere}.markdown-table th{font-size:11px;text-transform:none}.markdown-table tr{page-break-inside:avoid}.content{margin:13px 0;padding:12px;border:1px solid #dedbe7;border-radius:8px;break-inside:avoid}.content-markdown{break-inside:auto}.content img{max-width:100%;max-height:175mm}.content blockquote{margin:8px 0;padding:7px 10px;border-left:3px solid #6757df;background:#f5f3ff}.content pre{overflow-wrap:anywhere;padding:8px;background:#29273b;color:#fff;white-space:pre-wrap}.content code{font-family:monospace}.footer{margin-top:20px;padding-top:8px;border-top:1px solid #ddd;color:#777;font-size:9px}</style></head><body>'.$body.'</body></html>';
}
...
function pathway_page_pdf_item(PDO $pdo, int $itemId): ?array
{
    $query=$pdo->prepare('SELECT pi.*,p.title,p.summary,p.status,p.estimated_minutes,p.id AS page_id,c.title AS course_title,c.code AS course_code FROM pathway_items pi JOIN pages p ON p.id=pi.page_id JOIN courses c ON c.id=pi.course_id WHERE pi.id=?');
    $query->execute([$itemId]);
    $item=$query->fetch(PDO::FETCH_ASSOC);
    return $item?:null;
}

function pathway_page_pdf_render(PDO $pdo, array $item): string
{
    $itemId=(int)$item['id'];
    ...
    foreach($blocks->fetchAll(PDO::FETCH_ASSOC) as $block){$body.='<section class="content'.($block['type']==='markdown'?' content-markdown':'').'">';if($block['type']==='markdown')$body.=Qcm::renderStatic($block['body']);elseif($block['type']==='image')$body.='<img src="'.e(pdf_local_image($block['body'])).'" alt="'.e($block['image_alt']??$block['caption']).'"><p class="muted">'.e($block['caption']).'</p>';elseif($block['type']==='file')$body.='<strong>'.e(t('Fichier joint')).' :</strong> '.e($block['caption']?:basename($block['body'])).'<br><span class="muted">'.e($block['body']).'</span>';elseif($block['type']==='submission')$body.='<strong>'.e($block['caption']?:t('Travail à rendre')).'</strong>'.Markdown::render($block['body']).'<p>'.e(work_mode_label($block['submission_mode']).' · '.t($block['submission_required']?'Obligatoire':'Facultatif').' · '.t('Le texte est limité à 512 caractères.')).'</p>';else {$embed=Embed::parse($block['body']);$body.='<strong>'.e(t('Contenu intégré / vidéo')).' :</strong><br>'.($embed?'<a href="'.e($embed['url']).'">'.e($block['caption']?:($embed['title']?:$embed['url'])).'</a>':e(t('Intégration invalide : collez une URL http(s) ou un code iframe complet.')));} $body.='</section>';}
...
function student_pathway_page_pdf_html(PDO $pdo, int $itemId, int $studentId): string
{
...
function render_pdf_bytes(string $html,bool $landscape=false): string
{
    ...
    try{
        ...
        $pdf->WriteHTML('body{color:#29273b;font-family:dejavusans;font-size:10pt;line-height:1.45}h1{margin:0 0 5mm;font-size:22pt;color:#29273b}h2{margin:7mm 0 2.5mm;font-size:15pt;color:#29273b}h3{margin:5mm 0 2mm;font-size:12pt}h4{font-size:11pt}h5{font-size:10pt}h6{font-size:9pt}p{margin:1.5mm 0}.muted{color:#706e80}.header{margin-bottom:6mm;padding-bottom:4mm;border-bottom:1mm solid #6757df}.eyebrow{color:#6757df;font-size:8pt;font-weight:bold;letter-spacing:.7pt;text-transform:uppercase}.meta{margin-top:3mm}.pill{display:inline-block;padding:1.3mm 2.3mm;background-color:#ece9ff;font-size:8pt}.meta-separator{padding:0 1.5mm;color:#8b879c;font-weight:bold}.notice{margin:4mm 0;padding:3mm;border-left:1mm solid #6757df;background-color:#f5f3ff}.course-step-number{width:6%;text-align:center;white-space:nowrap}.course-skill-code{display:inline-block;margin-left:2mm;padding:.3mm 1.3mm;background-color:#ece9ff;color:#5344bb;font-size:7pt;font-weight:bold;white-space:nowrap}.course-reference-legend{padding:2mm 2.5mm;border-left:.8mm solid #6757df;background-color:#f5f3ff}.course-reference-legend p{margin:0}.course-objective-legend{margin-bottom:3.5mm}.course-skill-legend{margin-top:4mm}hr.markdown-separator{margin:5mm 0;border:0;border-top:.2mm solid #dcd9e5}.markdown-page-break{height:0;margin:0;border:0;page-break-after:always}table{width:100%;border-collapse:collapse}thead{display:table-header-group}th,td{padding:2.5mm;border:.2mm solid #dcd9e5;text-align:left;vertical-align:top}th{background-color:#ece9ff;font-size:8pt;text-transform:uppercase}tr:nth-child(even){background-color:#fafafd}.markdown-table-wrap{margin:3mm 0}.markdown-table{page-break-inside:auto}.markdown-table-weighted{table-layout:fixed}.markdown-table th,.markdown-table td{overflow-wrap:anywhere}.markdown-table th{font-size:8.5pt;text-transform:none}.markdown-table tr{page-break-inside:avoid}.content{margin:4mm 0;padding:4mm;border:.2mm solid #dedbe7;page-break-inside:avoid}.content-markdown{page-break-inside:auto}.content img{max-width:100%;max-height:175mm}.content blockquote{margin:2mm 0;padding:2mm 3mm;border-left:1mm solid #6757df;background-color:#f5f3ff}.content pre{padding:3mm;background-color:#29273b;color:#fff;white-space:pre-wrap}.content code{font-family:dejavusansmono}.footer{margin-top:7mm;padding-top:3mm;border-top:.2mm solid #ddd;color:#777;font-size:7pt}',\Mpdf\HTMLParserMode::HEADER_CSS);
