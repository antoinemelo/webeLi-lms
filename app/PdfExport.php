...
function course_pdf_html(PDO $pdo, int $courseId, int $teacherId): string
{
    ...
    $skillQuery=$pdo->prepare('SELECT i.pathway_item_id,pi.position AS item_position,s.id,s.code,s.title FROM item_skills i JOIN course_skills s ON s.id=i.skill_id JOIN pathway_items pi ON pi.id=i.pathway_item_id WHERE pi.course_id=?');$skillQuery->execute([$courseId]);$skillsByItem=[];$usedSkills=[];
...
function pathway_page_pdf_render(PDO $pdo, array $item): string
{
    ...
    foreach($blocks->fetchAll(PDO::FETCH_ASSOC) as $block){$body.='<section class="content'.($block['type']==='markdown'?' content-markdown':'').'">';if($block['type']==='markdown')$body.=Qcm::renderStatic($block['body']);elseif($block['type']==='image')$body.='<img src="'.e(pdf_local_image($block['body'])).'" alt="'.e($block['caption']).'"><p class="muted">'.e($block['caption']).'</p>';elseif($block['type']==='file')$body.='<strong>'.e(t('Fichier joint')).' :</strong> '.e($block['caption']?:basename($block['body'])).'<br><span class="muted">'.e($block['body']).'</span>';else $body.='<strong>'.e(t('Contenu intégré / vidéo')).' :</strong><br><span class="muted">'.e($block['body']).'</span>'; $body.='</section>';}
