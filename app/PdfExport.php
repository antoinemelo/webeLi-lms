...
function pathway_page_pdf_render(PDO $pdo, array $item): string
{
    ...
    foreach($blocks->fetchAll(PDO::FETCH_ASSOC) as $block){$body.='<section class="content">';if($block['type']==='markdown')$body.=Qcm::renderStatic($block['body']);elseif($block['type']==='image')$body.='<img src="'.e(pdf_local_image($block['body'])).'" alt="'.e($block['caption']).'"><p class="muted">'.e($block['caption']).'</p>';elseif($block['type']==='file')$body.='<strong>'.e(t('Fichier joint')).' :</strong> '.e($block['caption']?:basename($block['body'])).'<br><span class="muted">'.e($block['body']).'</span>';else $body.='<strong>'.e(t('Contenu intégré / vidéo')).' :</strong><br><span class="muted">'.e($block['body']).'</span>'; $body.='</section>';}
