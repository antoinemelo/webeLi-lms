...
function import_course_document(PDO $pdo, array $document, int $teacherId, string $mode, bool $resetDeadlines): int
{
    ...
    try{
        ...
        foreach($options['rewards']??[] as $reward){if(!is_array($reward)||trim((string)($reward['name']??''))==='')continue;$insertReward->execute([$courseId,trim((string)$reward['name']),trim((string)($reward['icon']??''))?:'✨',trim((string)($reward['color']??''))?:'#6d5dfc',max(1,(int)($reward['default_points']??1)),!empty($reward['active'])?1:0]);}
