...
function import_course_document(PDO $pdo, array $document, int $teacherId, string $mode, bool $resetDeadlines): int
{
    ...
    try{
        ...
        $insertItem=$pdo->prepare('INSERT INTO pathway_items(course_id,page_id,position,deadline,is_evaluation,self_evaluation_enabled,evaluation_weight,instructions,access_mode,framework_tracking_enabled) VALUES(?,?,?,?,?,?,?,?,?,?)');$linkSkill=$pdo->prepare('INSERT INTO item_skills(pathway_item_id,skill_id) VALUES(?,?)');
        foreach(array_values($items) as $index=>$item){$accessMode=in_array($item['access_mode']??'all',['all','restricted','none'],true)?(string)$item['access_mode']:'all';$frameworkTracking=$accessMode!=='none'||!array_key_exists('framework_tracking_enabled',$item)||!empty($item['framework_tracking_enabled']);$isEvaluation=!empty($item['is_evaluation']);$selfEvaluation=!array_key_exists('self_evaluation_enabled',$item)||!empty($item['self_evaluation_enabled']);$weight=normalize_evaluation_weight($item['evaluation_weight']??1);if($isEvaluation&&$weight===null)throw new TransferException('La pondération d’une évaluation est invalide.');$insertItem->execute([$courseId,$resolvedPages[$index],$index+1,$resetDeadlines?null:(trim((string)($item['deadline']??''))?:null),$isEvaluation?1:0,$selfEvaluation?1:0,$isEvaluation?$weight:1,(string)($item['instructions']??''),$accessMode,$frameworkTracking?1:0]);$itemId=(int)$pdo->lastInsertId();foreach((array)($item['skills']??[]) as $code)if(isset($skillMap[strtoupper((string)$code)]))$linkSkill->execute([$itemId,$skillMap[strtoupper((string)$code)]]);}
