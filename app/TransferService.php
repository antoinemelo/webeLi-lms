...
function export_course_document(PDO $pdo, int $courseId, int $teacherId, bool $includeOptions): array
{
    ...
    foreach($itemsQuery->fetchAll(PDO::FETCH_ASSOC) as $item){
        $exported=['page_reference'=>$item['page_reference'],'page_title'=>$item['page_title'],'position'=>(int)$item['position'],'deadline'=>$item['deadline'],'is_evaluation'=>(bool)$item['is_evaluation'],'self_evaluation_enabled'=>(bool)($item['self_evaluation_enabled']??1),'evaluation_weight'=>(float)($item['evaluation_weight']??1),'instructions'=>$item['instructions'],'access_mode'=>$item['access_mode']];
        if($includeOptions){
